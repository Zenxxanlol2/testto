    ...
    if Success and Data and Data.type == "auth_response" then
        ...
        if AuthSuccess then
            ...
            function GetRemoteEvent(remoteName)
                if not netenc1 or not netenc2 then
                    warn("Net encryption not found")
                    return nil, nil
                end
            ...
            local ValidRemotes = {
                ...
                TeleportService = {
                    Reconnect = "TeleportService/Reconnect",
                },
                CursedEventService = {
                ...
                QuantumCloner = {
                    OnTeleport = "QuantumCloner/OnTeleport",
                    UseItem = "UseItem"
                }
            }


            local UseItem, uiName = GetRemoteEvent(ValidRemotes.QuantumCloner.UseItem)
            local QuantumCloner, qcName = GetRemoteEvent(ValidRemotes.QuantumCloner.OnTeleport)
            ...
            local DefaultConfig = {
                Positions = {
                    ...
                    StealTracker = {X = 0.5, Y = 0.05},
                    CooldownTracker = {X = 0.02, Y = 0.55},
                    AutoBuy = {X = 0.02, Y = 0.3},
                }, 
                TpSettings = {
                    Tool           = "Flying Carpet",
                    Speed          = 2, 
            ...
            function instantClone()
                ...
                pcall(function()
                    ...
                    if UseItem and QuantumCloner then
                        UseItem:FireServer()
                        QuantumCloner:FireServer()
                    end
                end)
                _isCloning = false
                _cloneCooldownEnd = 0
            ...
            task.spawn(function()
                ...
                if not renderedMovingAnimals then
                    renderedMovingAnimals = Workspace:WaitForChild("RenderedMovingAnimals")
                end

                local function updateCharacter()
                    if LocalPlayer.Character then
                        currentCharacter = LocalPlayer.Character
                        humanoidRoot = currentCharacter:FindFirstChild("HumanoidRootPart")
                ...
                local function scanBestCarpet()
                    ...
                    for _, animal in ipairs(renderedMovingAnimals:GetChildren()) do
                        if animal:IsA("Model") then
                            local humanoidComp = animal:FindFirstChildOfClass("Humanoid")
                            if humanoidComp and humanoidComp.Health > 0 then
                                local value = getFinalGeneration(animal)
                                if value > bestValue then
                                    bestValue = value
                                    best = {
                                        name = animal.Name:gsub("_", " "),
                                        value = value,
                ...
                local function getAnimalPosition(animal)
                    ...
                    local part = animal:FindFirstChildWhichIsA("BasePart")
                    if part and part.Parent then
                        return part.Position
                    end
                    local humanoidComp = animal:FindFirstChildOfClass("Humanoid")
                    if humanoidComp and humanoidComp.RootPart then
                        return humanoidComp.RootPart.Position
                    end
                    return nil
                end
                ...
                local function lockToTarget()
                    ...
                    lockConnection = RunService.Heartbeat:Connect(function()
                        if not autoBuyEnabled or not isLocked or not currentTarget or not currentTarget.animal or not currentTarget.animal.Parent then
                            return
                        end
                        
                        if not humanoidRoot or not humanoidRoot.Parent then
                            updateCharacter()
                            return
                        end
                        
                        pcall(function()
                            local targetPos = getAnimalPosition(currentTarget.animal)
                            if targetPos then
                                humanoidRoot.CFrame = CFrame.new(targetPos + Vector3.new(0, 0, 0))
                ...
                local function spamE()
                    local now = tick()
                    if now - lastPromptTime >= 0.01 then
                ...
                RunService.Heartbeat:Connect(function()
                    ...
                    local best = scanBestCarpet()
                    if best then
                        currentTarget = best
                    end
                    
                    if not currentTarget or not currentTarget.animal or not currentTarget.animal.Parent then
                        unlockPosition()
                        return
                    end
                    ...
                    local animalPos = getAnimalPosition(currentTarget.animal)
                    if animalPos and humanoidRoot then
                        local distance = (humanoidRoot.Position - animalPos).Magnitude
                        if distance <= 15 then
                            spamE()
                        end
                    end
                ...
                UserInputService.InputChanged:Connect(function(input)
                    if dragEnabled and input.UserInputType == Enum.UserInputType.MouseMovement then
                        ...
                        frame.Position = UDim2.new(
                            math.clamp(startPos.X.Scale + delta.X / viewport.X, 0, 1 - PANEL_W / viewport.X), 0,
                            math.clamp(startPos.Y.Scale + delta.Y / viewport.Y, 0, 1 - PANEL_H / viewport.Y), 0
                        )
                    end
                end)
                ...
                minButton.MouseButton1Click:Connect(function()
                    ...
                    local targetSize = minimized and UDim2.new(0, PANEL_W, 0, 44) or UDim2.new(0, PANEL_W, 0, PANEL_H)
            ...
            task.spawn(function()
                ...
                local function updateUI(enabled, allPets)
                    ...
                    if SharedState.ListNeedsRedraw then
                        ...
                        if allPets and #allPets > 0 then
                            for i = 1, #allPets do
                                ...
                                local isPriority = petData.priorityIndex ~= math.huge
                                ...
                                valueLabel.TextColor3 = isPriority and Color3.fromRGB(100, 220, 130) or Color3.fromRGB(180, 220, 180)
                    ...
                    for i, pb in ipairs(petButtons) do
                        ...
                        if sel then
                            ...
                            pb.stroke.Color = pb.isPriority and Color3.fromRGB(80, 160, 100) or Color3.fromRGB(100, 100, 140)
                            pb.stroke.Transparency = 0.2
                            pb.name.TextColor3 = Color3.fromRGB(230, 230, 255)
                            pb.value.TextColor3 = pb.isPriority and Color3.fromRGB(80, 255, 130) or Color3.fromRGB(140, 240, 160)
                        else
                            ...
                            pb.value.TextColor3 = pb.isPriority and Color3.fromRGB(100, 220, 130) or Color3.fromRGB(130, 170, 130)
                        end
                    end
                        ...
                        if not hasShownPriorityAlert and Config.AlertsEnabled then
                            task.spawn(function()
                                ...
                                for i = 1, #priorityList do
                                    local searchName = priorityList[i]:lower()
                ...
                local function setupPlotListener(plot)
                    ...
                    scanSinglePlot(plot)
                    plot.DescendantAdded:Connect(function() task.wait(0.1); pcall(scanSinglePlot, plot) end)
                    plot.DescendantRemoving:Connect(function() task.wait(0.1); pcall(scanSinglePlot, plot) end)
                    task.spawn(function() while plot.Parent do task.wait(5); pcall(scanSinglePlot, plot) end end)
                end
                ...
                local function ShowPriorityAlert(brainrotName, genText, mutation, ownerUsername)
                    ...
                    local duelStatusColor = isInDuel and Color3.fromRGB(220, 100, 100) or Color3.fromRGB(100, 220, 140)

                    local MUT_ALERT_COLORS = {
                        Cursed = Color3.fromRGB(200, 0, 0),
                        Gold = Color3.fromRGB(255, 215, 0),
                        Diamond = Color3.fromRGB(0, 220, 255),
                        YinYang = Color3.fromRGB(200, 200, 220),
                        Rainbow = Color3.fromRGB(220, 100, 220),
                        Lava = Color3.fromRGB(255, 100, 20),
                        Candy = Color3.fromRGB(255, 105, 180),
                        Divine = Color3.fromRGB(255, 220, 100),
                        Bloodrot = Color3.fromRGB(160, 40, 40),
                        Radioactive = Color3.fromRGB(80, 220, 80),
                    }
                    local accentCol = (mutation and mutation ~= "None" and MUT_ALERT_COLORS[mutation])
                        or Color3.fromRGB(100, 200, 140)
                    ...
                    alertFrame.Size = UDim2.new(0, 360, 0, 80)
                    alertFrame.Position = UDim2.new(0.5, 0, 0, -90)
                    alertFrame.AnchorPoint = Vector2.new(0.5, 0)
                    alertFrame.BackgroundColor3 = Color3.fromRGB(12, 14, 18)
                    ...
                    local borderStroke = Instance.new("UIStroke", alertFrame)
                    borderStroke.Color = accentCol
                    borderStroke.Thickness = 1.5
                    borderStroke.Transparency = 0.3

                    local topAccent = Instance.new("Frame", alertFrame)
                    topAccent.Size = UDim2.new(1, 0, 0, 2)
                    topAccent.BackgroundColor3 = accentCol
                    topAccent.BorderSizePixel = 0
                    topAccent.ZIndex = 5
                    Instance.new("UICorner", topAccent).CornerRadius = UDim.new(0, 10)

                    local leftBar = Instance.new("Frame", alertFrame)
                    leftBar.Size = UDim2.new(0, 3, 1, -14)
                    leftBar.Position = UDim2.new(0, 10, 0, 7)
                    leftBar.BackgroundColor3 = accentCol
                    leftBar.BorderSizePixel = 0
                    Instance.new("UICorner", leftBar).CornerRadius = UDim.new(1, 0)

                    local tagLabel = Instance.new("TextLabel", alertFrame)
                    tagLabel.Size = UDim2.new(1, -30, 0, 16)
                    tagLabel.Position = UDim2.new(0, 20, 0, 9)
                    tagLabel.BackgroundTransparency = 1
                    tagLabel.Text = "PRIORITY DETECTED"
                    tagLabel.Font = Enum.Font.GothamBold
                    tagLabel.TextSize = 10
                    tagLabel.TextColor3 = accentCol
                    tagLabel.TextXAlignment = Enum.TextXAlignment.Left
                    tagLabel.TextTransparency = 0

                    local nameLabel = Instance.new("TextLabel", alertFrame)
                    nameLabel.Size = UDim2.new(1, -30, 0, 22)
                    nameLabel.Position = UDim2.new(0, 20, 0, 26)
                    nameLabel.BackgroundTransparency = 1
                    ...
                    genLabel.Size = UDim2.new(1, -30, 0, 16)
                    genLabel.Position = UDim2.new(0, 20, 0, 50)
                    genLabel.BackgroundTransparency = 1
                    genLabel.Text = genText .. "  ·  " .. (ownerUsername or "?") .. "  ·  " .. duelStatusText
                    genLabel.Font = Enum.Font.GothamMedium
                    genLabel.TextSize = 11
                    genLabel.TextColor3 = Color3.fromRGB(80, 220, 130)
                    genLabel.TextXAlignment = Enum.TextXAlignment.Left
                    genLabel.TextTransparency = 0

                    local duelDot = Instance.new("TextLabel", alertFrame)
                    duelDot.Size = UDim2.new(0, 60, 0, 14)
                    duelDot.Position = UDim2.new(1, -68, 0, 50)
                    duelDot.BackgroundTransparency = 1
                    duelDot.Text = duelStatusText
                    duelDot.Font = Enum.Font.GothamBold
                    duelDot.TextSize = 10
                    duelDot.TextColor3 = duelStatusColor
                    duelDot.TextXAlignment = Enum.TextXAlignment.Right
                    duelDot.TextTransparency = 0

                    TweenService:Create(alertFrame, TweenInfo.new(0.35, Enum.EasingStyle.Back, Enum.EasingDirection.Out), {
                    ...
                    task.delay(4, function()
                        if not alertGui.Parent then return end
                ...
                task.delay(0.5, function() SharedState.ListNeedsRedraw = true; updateUI(autoStealEnabled, get_all_pets()) end)
            ...
            task.spawn(function()
                ...
                frame.Size = UDim2.new(0, 450 * mobileScale, 0, 480 * mobileScale)
                ...
                ApplyViewportUIScale(frame, 450, 480, 0.45, 0.85)
                ...
                local function makeDraggable(dragFrame, targetFrame, configKey)
                    ...
                    dragFrame.InputBegan:Connect(function(input)
                        ...
                        if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
                            ...
                            input.Changed:Connect(function()
                                if input.UserInputState == Enum.UserInputState.End then
                                    dragging = false
                                    if Config.Positions and configKey then
                                        Config.Positions[configKey] = {
                                            X = targetFrame.Position.X.Scale,
                                            Y = targetFrame.Position.Y.Scale
                                        }
                                        SaveConfig()
                                    end
                                end
                    ...
                    dragFrame.InputChanged:Connect(function(input)
                        if dragging and (input.UserInputType == Enum.UserInputType.MouseMovement or input.UserInputType == Enum.UserInputType.Touch) then
                            ...
                            targetFrame.Position = UDim2.new(newX, 0, newY, 0)
                        end
                    end)
                end
                ...
                dragBar.ZIndex = 100
                makeDraggable(dragBar, frame, "AdminPanel")

                local controlBar = Instance.new("Frame", frame)
                controlBar.Size = UDim2.new(1, -20, 0, 80)
                controlBar.Position = UDim2.new(0, 10, 0, 10)
                ...
                proxCont.Position = UDim2.new(0, 10, 0, 100)
                ...
                local listFrame = Instance.new("ScrollingFrame", frame)
                listFrame.Size = UDim2.new(1, -20, 1, -160)
                ...
                local function teleportToPlayerPlot(targetPlayer)
                    if not targetPlayer then return end
                    
                    local plots = Workspace:FindFirstChild("Plots")
                    if not plots then 
                        ShowNotification("TP", "No plots found")
                        return 
                    end
                    
                    local targetPlot = nil
                    local deliveryHitboxPos = nil
                    local plotPos = nil
                    
                    for _, plot in ipairs(plots:GetChildren()) do
                        local success, ch = pcall(function()
                            return Synchronizer:Get(plot.Name)
                        end)
                        
                        if success and ch then
                            local owner = ch:Get("Owner")
                            if owner then
                                local isTarget = false
                                if typeof(owner) == "Instance" and owner:IsA("Player") then
                                    isTarget = (owner.UserId == targetPlayer.UserId)
                                ...
                                if isTarget then
                                    ...
                                    if deliveryHitbox and deliveryHitbox:IsA("BasePart") then
                                        deliveryHitboxPos = deliveryHitbox.Position
                                    end
                                    break
                                end
                            end
                    ...
                    if not targetPlot then
                        ShowNotification("TP", "Could not find " .. targetPlayer.Name .. "'s plot")
                        return
                    end
                    ...
                    if not hrp or not hum then
                        ShowNotification("TP", "No character found")
                        return
                    end
                    ...
                    local targetPos = deliveryHitboxPos or Vector3.new(plotPos.X, plotPos.Y + 5, plotPos.Z)
                    local targetHeight = targetPos.Y + 50
                    ...
                    ShowNotification("TP", "Teleported to " .. targetPlayer.Name .. "'s plot")
                end
                ...
                createPlayerRow = function(plr)
                    ...
                    btnCont.Size = UDim2.new(0, 180, 1, 0)
                    btnCont.Position = UDim2.new(1, -185, 0, 0)
                    ...
                    for i, def in ipairs(buttonsDef) do
                        ...
                        if def.cmd == "tp" then
                            b.BackgroundColor3 = S.Accent
                            b.BackgroundTransparency = 0.1
                            Instance.new("UICorner", b).CornerRadius = UDim.new(0, 10)
                            local bStroke = Instance.new("UIStroke", b)
                            bStroke.Color = S.Accent
                            bStroke.Thickness = 1.5
                            bStroke.Transparency = 0.3
                            
                            b.MouseButton1Click:Connect(function()
                                teleportToPlayerPlot(plr)
                            end)
                        else
            ...
            function runAutoSnipe()
                ...
                local isSecondFloor = itemHeight > 11 and itemHeight <= 23.15
                local isFirstFloor = itemHeight >= -6.9 and itemHeight <= 8.9
                local isThirdFloor = itemHeight > 23.15
                
                local plotIndex = getClosestBaseIdx(exactPos)
                local targetBasePos = nil
                
                if isSecondFloor then
                    targetBasePos = BASES_LOW[plotIndex]
                elseif isThirdFloor then
                    targetBasePos = BASES_HIGH[plotIndex]
                else
                    targetBasePos = BASES_LOW[plotIndex]
                end
                ...
                if isFirstFloor or isSecondFloor then
                ...
                if targetPlotUnlocked and (isFirstFloor or isSecondFloor) then
                    pcall(function()
                        ...
                        directHRP.CFrame = CFrame.new(exactPos.X, teleportY, exactPos.Z)
                        directHRP.AssemblyLinearVelocity = Vector3.zero
                        directHRP.AssemblyAngularVelocity = Vector3.zero
                        if isSecondFloor then
                            task.wait(0.05)
                ...
                if isThirdFloor or not targetPlotUnlocked then
                    local currentDistance = (hrp.Position - exactPos).Magnitude
                    walkForward(0.3)
                    task.wait(0.5)
                    ...
                    if inPlotRadius then
                        ...
                        if isFirstFloor then
                            for i = 1, 5 do
                                ...
                                newHRP.CFrame = CFrame.new(itemPos.X, -4, itemPos.Z)
                                newHRP.AssemblyLinearVelocity = Vector3.zero
                                newHRP.AssemblyAngularVelocity = Vector3.zero
                                task.wait(0.05)
                            end
                        elseif isSecondFloor then
                            for i = 1, 5 do
                                newHRP.AssemblyLinearVelocity = Vector3.zero
                                newHRP.AssemblyAngularVelocity = Vector3.zero
                                newHRP.CFrame = CFrame.new(itemPos.X, 2, itemPos.Z)
                                newHRP.AssemblyLinearVelocity = Vector3.zero
                                newHRP.AssemblyAngularVelocity = Vector3.zero
                                task.wait(0.05)
                            end
                        ...
                        elseif isThirdFloor then
                            for i = 1, 5 do
                                newHRP.AssemblyLinearVelocity = Vector3.zero
                                newHRP.AssemblyAngularVelocity = Vector3.zero
                                newHRP.CFrame = CFrame.new(itemPos.X, 21, itemPos.Z)
                                newHRP.AssemblyLinearVelocity = Vector3.zero
                                newHRP.AssemblyAngularVelocity = Vector3.zero
                                task.wait(0.05)
                            end
            ...
            function resetFindAndEquipFlying(character)
            ...
            function CreateToggleSwitch(parent, initialState, callback)
            ...
            task.spawn(function()
                ...
                local function createBrainrotBillboard(data)
                    ...
                    local displayName = (data.name or data.petName) or "???"
                    local genText = "★ " .. formatGenValue(data.genValue) .. "/s"
                    local mutText = hasMut and data.mutation:upper() or ""

                    local billboard = Instance.new("BillboardGui")
                    billboard.Name = "BrainrotESP_" .. data.uid
                    ...
                    local mutationLabel = Instance.new("TextLabel", billboard)
                    mutationLabel.Name = "MutationLabel"
                    mutationLabel.Size = UDim2.new(1, 0, 0.25, 0)
                    mutationLabel.Position = UDim2.new(0, 0, 0, 0)
                    mutationLabel.BackgroundTransparency = 1
                    mutationLabel.Text = mutText
                    ...
                    nameLabel.TextColor3 = hasMut and mutColor or Color3.fromRGB(255, 255, 255)
                    ...
                    local genLabel = Instance.new("TextLabel", billboard)
            ...
            function styleStroke(stroke, on)
                stroke.Transparency = on and 1 or 0.5
            end

            function createToggleRow(leftText, rightText, leftCallback, rightCallback, leftDefault, rightDefault)
                local row = Instance.new("Frame")
            ...
            function SliderRow(title, minVal, maxVal, defaultValue, callback)
            ...
            local kickISRow = createToggleRow("Auto Kick Steal", "Nothing",
                function(btn, stroke)
            ...
            function makeActionBtn(parent, text, xPos, callback)
                local btn = Instance.new("TextButton", parent)
            ...
            turnOff = function()
                ...
                local tracksToStop = tracks
                tracks = {}

                for _, t in pairs(tracksToStop) do
            ...
            task.spawn(function()
                ...
                local C_ACC = Color3.fromRGB(119, 120, 255)
                ...
                local HIGHLIGHT_NORMAL = Color3.fromRGB(119, 120, 255)
